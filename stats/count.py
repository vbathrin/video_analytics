                ...
                for i in count_zones:
                    if check_point_inside(temp, get_polygons(i)):
                        if temp["id"] not in blacklist:
                            ...
                            id_dict[person_id] = {}
                            id_dict[person_id]["start_time"] = str(
                                df.time[index])
                            id_dict[person_id]["zone_name"] = i["label"]
                        else:
                            id_dict[person_id]["end_time"] = str(
                                df.time[index])
        ...
        for j in dict_keys:
            if dwell_dict[j]["zone_name"] == i["label"]:
                if "end_time" in dwell_dict[j]:
                    total_time = total_time + (datetime.datetime.strptime(
                        dwell_dict[j]["end_time"], "%H:%M:%S.%f") - datetime.datetime.strptime(dwell_dict[j]["start_time"], "%H:%M:%S.%f"))
        if len(dict_keys) > 0:
...
def draw_count_zones(count_zones, img, identity=None):
    ...
    for i in count_zones:
        ...
        cv2.polylines(img, np.int32([polygon.exterior.coords]), 1, color=COLORS_10[identity % len(
            COLORS_10)] if identity is not None else COLORS_10[0], thickness=2)
...
def draw_dwell_zones(dwell_zones, img, identity=None):
    ...
    for i in dwell_zones:
        polygon = get_polygons(i)
        cv2.polylines(img, np.int32([polygon.exterior.coords]), 1, color=COLORS_10[identity % len(
            COLORS_10)] if identity is not None else COLORS_10[1], thickness=2)
